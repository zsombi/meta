...
/// %CoreSignal is the base class of the meta signals. Signals serve as observants in the observer
/// pattern. This class defines the core behavior of a meta signal.
/// \see Signal
class META_API CoreSignal
{
    DISABLE_COPY(CoreSignal);
    DISABLE_MOVE(CoreSignal);
...
public:
    ...
    /// \return If the connection suceeds, returns the slot, otherwise returns \e nullptr.
    template <typename Function>
    typename std::enable_if<!std::is_base_of_v<meta::CoreSignal, Function>, Slot*>::type
    connect(Function f)
    {
...
protected:
    /// Constructor.
    explicit CoreSignal() = default;
    /// Destructor.
    virtual ~CoreSignal() = default;
...
/// Signal template, specialization with a signature of void function with arbitrary arguments. You
...
class META_TEMPLATE_API Signal<void(Arguments...)> : public CoreSignal
{
public:
...
protected:
    /// Overrides CoreSignal::verifySignature().
